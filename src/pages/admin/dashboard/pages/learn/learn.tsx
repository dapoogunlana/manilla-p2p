...
import EditLearnModal from '../../../../../components/block-components/edit-post-modal/edit-post-modal';
import './learn.scss';



interface iLearnPost {
  body: any[]
  brief: string;
  datePosted: string;
  id: number;
...
function Learn(props: any) {
  ...
  const [learnPosts, setLearnPosts] = useState<iLearnPost[]>([]);
  ...
  const getLearnPosts = () => {
    ...
    sendRequest({
      ...
      // this.learnPosts = posts.map((item) => {
      //   if (item.image) {
  ...
  const removeImage = (post: iLearnPost) => {
    setLoading(true);
    ...
    sendRequest({
      url: `learn/image/${post._id}${format}`,
      method: 'DELETE'
    }
    , (res: any) => {
      toast.success(res.message);
  ...
  const openEditModal = (post: iLearnPost) => {
    setActivePost(post);
  ...
  return (
    <>
            ...
            <div className="col-lg-12" data-aos="zoom-in" data-aos-delay="400">
                <div className="card-hover">
                    <div className="db-card-body cover">
                      ...
                      <form method="POST" className="register-form" name="news_form" onSubmit={createLearnPost} id="news_form">
            ...
            <div className="col-lg-12" data-aos="zoom-in">
                {
                  learnPosts.length === 0 && <>
                    {
                      loading ?
                      <div className='main-card py-5 mt-4'>
                        <p className='py-5'></p>
                        <div className=""><MiniLoader/></div> 
                        <p className='py-5'></p>
                      </div> :
                      <div className='main-card pt-3 mt-4'>
      ...
      {
        editModalVisible && <EditLearnModal editKey={'learn'} post={activePost} closeModal={closeEditModal} />
      }
    </>
  );
}
...
//       </div>
//       {
//         editModalVisible && <EditPostModal editKey={'learn'} post={activePost} closeModal={closeEditModal} />
//       }
//     </>
//   );
// }
